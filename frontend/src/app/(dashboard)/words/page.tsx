...
async function WordsGrid({ language }: { language?: string }) {
  const { data: words } = await wordsApi.list({ language, limit: 48 });
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {words.map((w: Word) => <WordCard key={w.id} word={w} />)}
    </div>
  );
}
...
export default async function WordsPage({ searchParams }: Props) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      <p className="text-sm text-surface-400 mb-6">Duyệt và tìm kiếm toàn bộ ngân hàng từ.</p>
