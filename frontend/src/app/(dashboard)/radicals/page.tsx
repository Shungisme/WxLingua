...
export default async function RadicalsPage() {
  const { data: radicals } = await radicalsApi.list({ limit: 214 });

  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <h1 className="text-2xl font-bold text-surface-900 mb-1">Bộ thủ Khang Hi</h1>
      ...
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
        {radicals.map((r: Radical) => {
          const meaning = (r.meaning as Record<string, string>)?.vi ?? (r.meaning as Record<string, string>)?.en ?? '';
          return (
            <div
              key={r.id}
              title={meaning}
            ...
            >
              <span className="text-2xl text-surface-900 leading-none">{r.char}</span>
