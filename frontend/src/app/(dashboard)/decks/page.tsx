...
async function DeckGrid() {
  const { data: decks } = await decksApi.list({ public: true });
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {decks.map((d: Deck) => <DeckCard key={d.id} deck={d} />)}
    </div>
  );
}
...
export default function DecksPage() {
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <p className="text-sm text-surface-400 mb-8">Khám phá các bộ thẻ cộng đồng.</p>
      <Suspense
        fallback={
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
